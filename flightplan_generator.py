    ...
    def __init__(self, knowns, k=3):
        '''Args:
            knowns [array]: numpy array of containing, for each coordinate on the known line,
                ie, the end of the path before and the start of the next path
...
def look_at_vectors(path_coords, target_coords, weights):
    '''Generates the look at vectors, given the coords of the path, the coord of the lookat targets,
    and the weighting towards the primary target. This is a method of interpolating the look vectors 
    of the camera. In areas where we aren't looking specifically at a galaxy, it interpolates looking at
    the previous and the next. We do this by specifying a priamry target, and secondary for each point on the path
    and a weight of how heavily we favour the primary. On scripted sections of the path, ie not splined, the weight is
    typically 1, but on splined paths it tends towards 0 and the rate it does this determines the speed we turn around'
    Args:
        path_coords [array]: The array of coords for each frame on the path, [x,y,z].
        target_coords [array]: The array of primary and secondary targets. [primary_x,y,z secondary_x,y,z]'''
...
def gen_centre_bundle(frames, coords, const=True):
    '''This is intended to interpolate the centre position of the galaxy for the purpose of orbits
        that follow the galaxy. This is not currently set up, and we assume constant position
        ---TODO: Make this work'''
    if const:
...
class CombinedPath(object):
    def __init__(self, func_domain):
        '''Sets up a new path function, as a piecewise of component paths.
        Args:
            func_domain [array]: List of function domains, and path functions for those
                domains. Of the form [start_frame, end_frame, path_function]'''
        self.func_domain = func_domain
        self.fix_domain_holes()
        #print self.func_domain[:,1]

    def fix_domain_holes(self):
        '''Internal function to fix the holes in the domains. It detects which frames do not have functions
        defined for them, and fills the gap with a spline.'''
    ...
    def __call__(self, frames):
        '''Use this class as a path function. For given frame values, it will apply a pathfunction
        to those frames to get coords. It chooses the path function that matches the domain
        Args:   
            frames [array]: Frame or set of frames to gen coords for
        Returns:
            coords [array]: Set of coords corrosponding to those frames '''
        conditions = []
...
def create_flight_path(inp_data, mult_h, fname):
    '''Creates a flight path from the data laid out in inp_data, and saves it as a txt under fname
    All distances are in cMpc, all time in frames.
    Args:
        inp_data [array]: Numpy array with a specific layout, typically from the GUI data tables. I will
            try document it below, here goes:
            inp data is an array of partial path entries, of the form
            [st_fr,en_fr, st_sf,en_sf, targ_x,targ_y,targ_z, axis_x,axis_y,axis_z, rv,av, ro,ao, hv,ho]
            where those variables are defined as follows.
            st_fr: The start frame of the partial path. For the very first section, this must begin at -1
            en_fr: The end frame of the partial path.
            st_sf: The scale factor at the begining of the frames
            en_sf: The scale factor at the end of the frames
            targ_x/y/z: The coordinate at the centre of the parial path, also the point of interest we look at
            axis_x/y/z: The direction vector of the orbital axis, for the parial path. To orbit about the z_axis 
                for example, this would be (0,0,1)
            Note, the next parameters are working in the frame of reference of the target object, and are in 
            cylinderical coords.
            rv: Radial velocity, for circular paths of constant radius, set this 0, else you get spirals.
                in units cMpc/frame
            av: Angular velocity, for circular paths set this as non zero, for lines of various sorts it can take 0.
                in units orbits/frame
            ro: Initial radius of the orbital path in units cMpc
            ao: Initial angle, used to shift the orbit around to tweak paths in unit orbits
            hv: Helix veloicty, the velocity in the direction of the rotational axis. Used to make paths straight past
                obects and helixes in units cMpc/frame
            ho: Initial axis height in units cMpc
    '''
    ...
    print "\nFlight file generated under path: %s" % fname
    return True
...
if __name__ == "__main__":
    ...
    the derivative of the 0th frame, we need to look back to a position at negative frames, and this needs
    to be contained within a paths domain. ;(
    '''
